...
#include <stdio.h>
#include <D:\exe\RT-ThreadStudio\workspace\h743_400_led_mpu_lwip2.0.1_ptpmtest\applications\ptp\dep\constants_dep.h>
...
static ssize_t netRecv(octet_t *buf, TimeInternal *time, BufQueue *msgQueue)//接受方向
{
	int i;
	int j;
	u16_t length;
	...
	if (time != NULL)
	{
	   ...
	   // printf("\n  rx three %lu.%lu\n\n",eth_timestamp.rdes1,eth_timestamp.rdes0);
	    time->seconds=eth_timestamp.rdes1;
	    time->nanoseconds = eth_timestamp.rdes0;
...
static ssize_t netSend(const octet_t *buf, int16_t  length, TimeInternal *time, const int32_t * addr, struct udp_pcb * pcb)//发送方向
{
	err_t result;
	struct pbuf * p;
	...
	{
#if LWIP_PTP==1//发送
	     //printf("\n  tx three %lu.%lu\n\n",eth_timestamp.tdes1,eth_timestamp.tdes0);
	     time->seconds=eth_timestamp.tdes1;
	     time->nanoseconds = eth_timestamp.tdes0;
#else
		getTime(time);
#endif
